          ...
          <div class="col-md-9 order-2 filtered" style="margin-top:-10px;">
            ...
            <div class="row" style="display:block">
              ...
              <div class="col-lg-4 col-md-6 item-entry mb-4 bg-white" style="display:inline-block;padding-bottom:27px;">
                <a href="/shops?id={{$prod->product_id}}" class="product-item md-height  d-block">
                <img src="images/products/product_{{$prod->product_id}}/small/{{$prod->product_image}}" alt="Image" class="img-fluid">
                </a>
                <h2 class="item-title"><a href="/shops?id={{$prod->product_id}}">{{$prod->product_name}}</a></h2>
                @if ($prod->product_new_price)
                 <strong class="item-price"><del>{{$prod->product_price.' Lei  '}}</del>{{$prod->product_new_price.' Lei'}}</strong>
                @else
                <strong class="item-price">{{$prod->product_price.' Lei'}}</strong>
                @endif
              </div>
            ...
            <div class="row">
              <div class="col-md-12 text-center">
                <div class="site-block-27">
                  <ul>
                   @if(count($products)>0)
                    @if($active_tab > 0)  <li><a class="prod-nav" next="{{$active_tab-1+1}}">&lt;</a></li> @endif
                    <?php  
                          for($i = 0; $i<$pages; $i++){
                    ?>
                    @if($i == $active_tab)
                          <li class="active"><span>{{$active_tab+1}}</span></li>
                    @else
                          <li><a class="prod-nav" next="{{$i+1}}">{{$i+1}}</a></li>
                    @endif
                    
                          <?php } ?>
                          @if($active_tab != $pages-1) <li><a class="prod-nav" next="{{$active_tab+2}}">&gt;</a></li> @endif
                    @else
            ...
            <div class="border p-4 rounded mb-4width sel-container">
              ...
              <ul class="list-unstyled mb-0 unselected-cats">
               <?php foreach ($cats as $cat=>$props) { ?>
                <li class="mb-1 cats" id="cat_{{$props["id"]}}"><a class="d-flex text-black"><span>{{$cat}}</span> <span class="text-black ml-auto">({{$props["count"]}})</span></a></li>
               <?php }?>
              </ul>
            ...
            <div class="border p-4 rounded mb-4 sel-container">
              ...
              <div class="mb-4 size-container">
                ...
                <?php foreach($sizes as $size){ ?>
                  ...
                  <input type="checkbox" id="s_{{$size->size_id}}" class="mr-2 mt-1"> <span class="text-black">{{$size->description}} ({{$size->size_count}})</span>
                  </label>
                <?php } ?>
              </div>
              ...
              <div class="mb-4 unselected-tags">
                ...
                <?php foreach($tags as $tag){?>
                  <a  class="d-inline-flex color-item align-items-center tag" style="margin: 0 5px;" id="tag_{{$tag->tag_id}}">
                  </span> <span class="text-black">#{{$tag->name}} ({{$tag->tag_count}})</span>
                </a>
              <?php } ?>
